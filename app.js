...
//   'local-server'   → node server.js (recommended, key in .env)
//   'openai'         → OpenAI direct  (paste key in AI_KEY)
//   'doubao'         → Doubao/ByteDance Ark API
//   'openai-compat'  → Any OpenAI-compatible API (DeepSeek, Moonshot…)

const AI_PROVIDER        = 'local-server';
const AI_KEY             = '';          // only for non-server modes
const AI_MODEL           = '';          // leave blank for defaults
const AI_ENDPOINT_COMPAT = '';          // base URL for openai-compat mode
...
function initInputModal() {
  ...
  document.getElementById('story-char-count').textContent = '0/1000';
  // render quick-tag rows
  renderQuickTags();
}

function updateCharCount() {
  const val = document.getElementById('story-input').value;
  currentStory = val;
  document.getElementById('story-char-count').textContent = val.length + '/1000';
}
...
function fillStoryPrompt(text) {
  ...
  ta.value = text;
  currentStory = text;
  document.getElementById('story-char-count').textContent = text.length + '/1000';
  ta.focus();
}
...
function renderEntries() {
  ...
  const daysInMon  = new Date(todayYear, todayMonth + 1, 0).getDate();
  const d          = getData(currentUser);
  const cover      = d.cover || {};

  for (let i = 0; i < firstDay; i++) grid.appendChild(document.createElement('div'));
  for (let dy = 1; dy <= daysInMon; dy++) {
    const el = document.createElement('div');
    el.className = 'e-cal-day';
    el.textContent = dy;
    if (cover[dy])  el.classList.add('has-entry');
    if (dy === todayD) el.classList.add('today');
    grid.appendChild(el);
  }
  ...
  if (tabWrap) {
    const prev = (todayMonth - 1 + 12) % 12;
    tabWrap.innerHTML = `
      <div class="month-tab" onclick="this.classList.add('active');this.nextElementSibling?.classList.remove('active')">${months[prev]} ${prev > todayMonth ? todayYear - 1 : todayYear}</div>
      <div class="month-tab active">${months[todayMonth]} ${todayYear}</div>`;
  }
}
